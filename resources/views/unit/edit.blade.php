                ...
                <form role="form" method="POST" action="{{ route('unit.update',$unit->id) }}"
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group row">
                            <label class="col-sm-3" for="regiment_id">Relevant Regiment</label>
                            <div class="col-sm-9">
                                <select required name="regiment_id" id="regiment_id"
                                    ...
                                    <option selected value="">Choose a Relevant Regiment</option>
                                    @foreach($regiments as $regiment)
                                        <option
                                            @selected($regiment->id == $unit->regiment_id) value="{{$regiment->id}}">{{$regiment->name}}</option>
                                    @endforeach
                                </select>
                                @error('regiment_id')
